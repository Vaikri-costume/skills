...
**`author.primary` is checked, not just preferred (confirmed 2026-08-17).** `attribution_lint.py`'s `lint_author_identity` compares `author.primary` against `gh api user -q .login` (the CLI's own authenticated identity) whenever `gh` is installed and authenticated — a mismatch is a **blocking** violation, not a documentation nicety. This exists because a shape check alone can't catch a bare first name (e.g. `"Vaidehi"`): it's syntactically a valid-looking identifier, so only a real identity comparison catches it. When `gh` is unavailable or unauthenticated there's nothing to verify against, so the check degrades to advisory (never blocks on an environment gap) — but whenever `gh` IS available, set `author.primary` to the real GitHub username, not a name of convenience.
