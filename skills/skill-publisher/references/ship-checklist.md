...
**On the description-quality row:** it is judged by the cold CCVW audit (Step 3), backed by two deterministic signals from `scripts/triggering_eval.py`. (1) **Always-on confidence heuristic** — boundary-clause (P1b "Do NOT use …") + WHEN-signal presence + length, yielding `confidence` high/medium/low. This is **advisory** (the same field `--readiness` surfaces): low confidence reinforces an audit description GAP but never on its own becomes a cluster. (2) **Opt-in measured accuracy** (`--run-eval`, when `<triggering-eval>` is true) — `triggering_eval.py` delegates to `skill-creator-ccvw`'s `run_eval.py` against a `{query, should_trigger}` set; below the accuracy threshold (exit 1) → an AUDIT cluster (flag `trigger-accuracy`) FIXed in Step 5 by rewriting the description (optionally via ccvw's `run_loop.py` / `improve_description.py`, applying the returned `best_description`). When the eval can't run (no query set / ccvw absent / harness error → exit 3) it degrades to the heuristic and writes **no** cluster — a broken environment is not a description defect. (Exit 2 = a usage/path error to fix. And if `<triggering-eval>` was requested via natural language but **no `--eval-set` query set was supplied**, do NOT invoke `--run-eval` with an empty path — that exits 2; degrade to the heuristic-only result and tell the user the measured eval needs a `{query, should_trigger}` set, naming the `--eval-set` argument they can supply. No cluster.) A weak description (missing WHEN, no trigger phrases, too vague/too technical, no file-type mention when format-specific) becomes an AUDIT cluster addressed FIX/STRENGTHEN/USER-PAUSE like any other. If the description is sound — no audit GAP and (when run) accuracy ≥ threshold — there is no cluster and therefore no ledger row (`append_ledger.py` rows require a real finding's flag ID, which a no-finding re-validation does not have); note "description re-validated for triggering" in your response text instead, and move on.
