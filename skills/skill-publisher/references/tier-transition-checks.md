...
## Shared-script sync checks

**RETIRED (2026-06-20).** Several scripts used to be kept byte-for-byte in sync across the sibling skills — the publisher's `portability_lint`/`attribution_lint`/`quick_validate` with skill-creator-ccvw, and `render_ledger`/`append_ledger` with skill-tracer. **That contract is gone.** skill-publisher carries its **own independent copies** and they may freely diverge: drift is no longer a finding, and the ship flow runs **no** sync check. There is nothing to do at Step 4 for shared scripts — just run the publisher's own copies. (`scripts/check_shared_sync.py` is retained only as a dormant manual tool for anyone who wants to eyeball the behavioral diff between copies — e.g. to hand-port a fix from one skill to another — but it is not invoked anywhere and a non-zero exit is not something to "fix".)
...
Before the tier-specific checks below, the frontmatter must be valid and structurally correct, via two of the publisher's own scripts (`quick_validate.py` + `portability_lint.py` — independent copies; skill-creator-ccvw has its own, see the retirement note above). `<target>` in all script commands below is the **absolute path to the skill root** being shipped (e.g. `~/.claude/skills/my-skill/`); use it consistently across calls since Bash cwd does not persist between tool calls:
...
## Internal-link integrity gate (claude-users+)

`link_check.py <target>` detects three distinct CCVW-skill defects in one pass:

```bash
python3 ~/.claude/skills/skill-publisher/scripts/link_check.py <target> --json   # exit 0 clean / 1 findings / 2 path error
```

- **Broken link (blocking TIER cluster).** A `references/*.md` / `scripts/*.py` / `assets/*` path cited in SKILL.md or a reference that does not resolve on disk. The skill's own polish pattern — move a section to `references/<topic>.md`, leave a pointer — orphans these, and nothing else verifies the target exists. The executor would try to load a missing file, so each unresolved citation is a blocking cluster (`broken_links[]` in the JSON; one cluster row per entry).
- **Unreadable file (blocking TIER cluster).** A cited file that exists but could not be read — a permission or encoding fault (`unreadable[]` in the JSON). The scan could not inspect it, so a clean verdict would be a false negative; surface it as blocking and resolve the file before shipping. (`readiness_report.py`'s `_gate_links` classifies `unreadable` as a blocker for the same reason.)
- **Dead script (advisory TIER cluster).** A `scripts/*.py` that nothing references or imports (`dead_scripts[]`). Dead code shipped to users; advisory — remove the script or wire it into the workflow.

**Cross-skill citations are auto-excluded.** A citation in a paragraph that names a sibling skill (any `skill-<name>` token other than this skill's own folder name — e.g. skill-creator-ccvw's `references/portability-spec.md`, or a `~/.claude/skills/skill-tracer/scripts/...` absolute path) points at another skill's tree and is NOT flagged. So a `broken_links` finding is genuinely this skill's orphaned link. (Limitation: a real internal orphan that happens to sit in a paragraph also naming a sibling skill is a false negative — the common case, a stale pointer in a self-only paragraph, is still caught.)

Add each `broken_links` entry and each `unreadable` entry to `<tier-clusters>` as a blocking cluster, and each `dead_scripts` entry as an advisory cluster; Step 5 addresses them with the rest.
