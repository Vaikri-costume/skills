...
**Unknown action keyword (typo, future variant, manual edit)**: treat as if it were `dispatch round-N` (the most conservative recovery — re-checks the JSONL for prior dispatches; if none found, re-dispatches cold). Tell the user: "Unknown in-flight action `<word>` at round N — treating as dispatch. If you intended a different recovery, edit the marker before re-invoking." **Truncated-marker sub-case (`round: null`) — takes precedence over the keyword check, regardless of `action_valid`:** if `ledger_state.py` reports the marker has no usable round number — whether because the marker is truncated (e.g. `in-flight:: <Runtime>` — `action: null, round: null`) OR because the round token is unparseable (e.g. `in-flight:: <Runtime> dispatch roundN` — `action_valid: true` but `round: null`, since `roundN` doesn't match `round-<int>`) — there is no round to resume on. Discard the unusable marker, tell the user "Incomplete in-flight marker `<raw>` — discarding and starting fresh", and proceed as if no marker were present: apply rule 4 (no prior rounds) or rule 5 (prior rounds exist), deriving the round from `highest_round`.
...
The current working directory with **every non-alphanumeric character replaced by `-`** (not just `/` — also spaces, `.`, `_`, etc.), which for an absolute path yields a leading `-`. `recover_dispatch.py`'s `encoded_cwd()` implements exactly this (`re.sub(r"[^A-Za-z0-9]", "-", os.getcwd())`).

Example: `/Users/yourname/Information for LLMs` → `-Users-yourname-Information-for-LLMs` (note the spaces become `-`, not just the slashes — replacing only `/` would mis-encode any cwd containing a space or `.` and point auto-location at the wrong project dir).
...
1. **`in-flight:: <Runtime> dispatch round-N`** — mid-dispatch interrupted. Recover the prior session's dispatch results from the session JSONL with `scripts/recover_dispatch.py` rather than hand-parsing raw JSONL (hand-scanning is silently error-prone — easy to grab a discarded retry or a backgrounded agent's launch-ack instead of the real result):
   ...
   # NOTE: <encoded-cwd> is derived from the LIVE cwd at recovery time. If the cwd differs from the one
   # active when the trace was originally dispatched, auto-location points at the wrong project dir —
   # pass --project-dir <dir> or --jsonl <path> explicitly in that case.
   ```

   It matches the three constant description strings (`Forward trace of <skill>`, `Backward trace of <skill>`, `Executor trace of <skill>` — skill-name match filters out unrelated dispatches), pairs each `tool_use` to its `tool_result` (handling the background-agent `<task-notification>` case), applies **most-recent-wins** so a discard-and-retry selects the retry not the stale dispatch, and returns JSON: `found` (per-direction `tuid` + `dispatch_line` + `result_line`), `missing` (which of forward/backward/executor have no usable result), and `result_text` (the recovered report per direction — **only for directions actually recovered; a direction in `missing`, including one that appears in `found` with `result_line: null`, has NO `result_text` entry**, so drive re-dispatch off `missing`, never off iterating `result_text`). Exit 0 = all three recovered; exit 1 = one or more missing; exit 2 = the project dir or newest JSONL couldn't be located (nothing to recover from) — re-dispatch all three cold, and tell the user if the project dir looks wrong.
...
2. **`in-flight:: <Runtime> addressing round-N`** — mid-Step-6 interrupted. Filter the ledger's rows by Round = N — the present rows are the addressed clusters. The `<flag-to-cluster-map>` (Step 5's flag→cluster map) is in-memory only, so this recovery must rebuild it: re-extract the trace agents' reports from the prior session's JSONL (rule 1's JSONL-parse procedure applies), then re-run Step 5's clustering procedure cold. Match the re-derived clusters against the addressed rows by Root cause + Flags — the first re-derived cluster whose row is not on the ledger is the next one to address. If no Round = N rows exist yet (interruption between the `addressing` marker write and the first row append), start at C1 of the re-derived cluster set. Rows for remaining clusters use the **prior session's Runtime** (same round, same Runtime). Round = N from the marker.
   ...
   - **Code-review phase of round 1 (SKILL.md Step 2.5).** The code-review phase is the *first phase of round N* (it shares the round with the cold trace) and also writes an `addressing round-N` marker before applying its `CR*` fixes. Two cases, distinguished by the JSONL and the `REVIEW` rows:
     - **Trace phase already started** — the JSONL holds a `Forward/Backward/Executor trace of <skill>` dispatch for round N: the code-review phase finished (its `REVIEW` rows are on the ledger) and the interruption is in the *trace* addressing. Recover exactly as the normal rule-2 path above (rebuild the `TRACE` clusters from the JSONL, match against the round-N `TRACE` rows, address the remaining ones). **Leave the existing `REVIEW` rows alone** — they are already addressed; re-derived `TRACE` clusters won't match them.
     - **Code-review phase interrupted** — **no** `Forward/Backward/Executor trace of <skill>` dispatch exists for round N (and round N's rows, if any, are all Phase `REVIEW`, or there are none yet): re-run Step 2.5(a) from the top against the *current* files — this re-enters its full path decision: prefer `/code-review max`, and if it still can't be scoped to whole files, the fallback general-purpose Agent runs again (the review is idempotent — re-reading current files; an interrupted fallback review Agent's *partial* findings are **not** recovered — `recover_dispatch.py` does not match its `Code-review pass of <skill>` description — but that is fine, the idempotent re-run re-derives them. Recovery keys on the **absence of a `Forward/Backward/Executor trace` dispatch + the `REVIEW`-only rows**, not on the review Agent's description) — then re-cluster with the `CR` prefix, and address only the clusters not already on the ledger for round N (same Root-cause + Flags match, against the present `REVIEW` rows). Then continue with Step 2.5(d) — proceed into the cold trace (Step 3 → Step 4) for the **same** round N; do NOT close the round here.
     - Rows use the **prior session's Runtime**; Round = N from the marker in both cases.
...
4. **No `in-flight::` is present and no prior rounds exist (ledger absent or empty)**: this is the skill's first trace. Round = 1 (set `<round-number> = 1` — the same variable rule 5 sets explicitly; SKILL.md Step 1's "after the rule runs, `<round-number>` is set" depends on this assignment). Use current invocation's Runtime. Create the ledger at `~/.claude/skill-tracer-audit-ledger/<target-skill-name>.md` using ledger-format.md "Header template (new ledger)" format. Proceed to Step 2 → **Step 2.5** → Step 3 → Step 4 (Step 2.5 runs first iff its own conditions hold — SKILL.md Step 2.5 "When this runs" owns that decision, recovery does not re-implement it; Step 4 then writes the first `dispatch round-1` marker, overwriting any `addressing round-1` Step 2.5 wrote).

5. **No `in-flight::` is present but prior rounds exist in the ledger**: Before proceeding on either path (clean or not-clean), **`<round-number>` = highest existing Round + 1 is set immediately here** — this IS rule 5's action at Step 1(a), computable from `ledger_state.py`'s `highest_round` with no need for later steps (this is what "`<round-number>` set at (a)" means in SKILL.md). The in-flight marker is written later, at Step 4's atomic-write (`dispatch round-<N+1>`), after Steps 2–3 run — but it carries the round number already set here. So for the not-clean sub-case: set `<round-number>`, then proceed to Step 2 → Step 3 → Step 4. For the clean sub-case (user accepts): same path. Ambiguous state — the prior session either completed all its rounds (converged or stopped) but never wrote a handoff marker, OR crashed in a way that lost the marker. To distinguish: check whether the most recent prior round was clean (see "Definition: 'round was clean'" below — `ledger_state.py`'s `last_round_clean` is `true` / `false` / `null`, and **`null` (cleanliness unknown — the highest round has no summary comment) is treated as not-clean**, the same as `false`). If clean (`true`) → ask the user whether to start a fresh round (the trace previously converged; new round may be unnecessary). If user accepts (y): start a fresh round at Round = (highest existing Round + 1), using current invocation's Runtime, and proceed to Step 2 → **Step 2.5** → Step 3 → Step 4. (This is the post-convergence re-trace; whether Step 2.5's code-review phase runs is **Step 2.5 condition 3's decision** — recovery does not re-implement it.) If user declines (n): exit Step 1 without proceeding (see "Definition: round was clean" below for the exact exit). If not clean → start a fresh round at Round = (highest existing Round + 1) using current invocation's Runtime, proceed to Step 2 → Step 3 → Step 4 (Step 2.5 does NOT run — its condition 2 excludes rule 5's not-clean sub-case). Do NOT ask about "fresh or continue" in the not-clean case — prior unaddressed clusters in the ledger combined with no handoff marker is a clear signal that work remains.

---

## Definition: "round was clean"

A round is clean when its **cold trace returned zero `TRACE`-phase clusters** — i.e. the round closed (has a round-summary comment) and has **zero `TRACE`-phase data rows**. This matches Condition A (convergence is defined over the cold trace). It is deliberately **phase-aware, not a raw-flag-count test**: a round-1 that converges *with* a code-review phase carries `REVIEW` (`CR*`) rows, so its summary's total `raw flags` is > 0 even though the cold trace was clean — keying on `raw flags 0` would mis-classify that converged round as not-clean. `ledger_state.py` computes this as its **`last_round_clean`** field (`true` = highest closed round has zero `TRACE` rows; `false` = it has `TRACE` rows; `null` = the highest round has no summary, i.e. didn't close / cleanliness unknown); the orchestrator reads that field directly rather than re-scanning by hand — this definition is what the field means.
...
|---|---|---|
| Step 2.5 code-review addressing (round 1 only — full-convergence) | `in-flight:: <Runtime> addressing round-1` (written on entering the code-review addressing work, **before** the round's first dispatch — the one case where `addressing` precedes `dispatch`; see SKILL.md Step 2.5(b)) | Overwritten by `dispatch round-1` at the same round's Step 4. A recovery seeing `addressing round-1` distinguishes this pre-dispatch case from Step 6's post-dispatch addressing by whether a `Forward/Backward/Executor trace` dispatch exists in the JSONL — rule 2's code-review-phase branch (above) |
...
1. **Arm on first dispatch — ASK the user for the first interval.** **First, test whether a resume mechanism is actually available:** the harness `/loop` mechanism (you cannot probe `/loop` directly — treat it as available only if *this* invocation was itself launched via `/loop`), or the `mcp__scheduled-tasks__create_scheduled_task` tool (testable: is it in your toolset?). If you cannot confirm either, treat the mechanism as **unavailable** and **skip arming** — arming against a mechanism that can't fire leaves an orphaned expectation of autonomous resume that never comes; resume then relies on manual re-invocation. If a mechanism IS available: when Step 4 writes the first `dispatch round-N` marker of an invocation, ask the user how long out to set the first resume wakeup (e.g. "How long should I wait before an autonomous resume-wakeup fires if this trace gets interrupted?"), then schedule a one-shot wakeup at `now + <that interval>`. **Convert the answer to the absolute `fireAt` the tool needs:** parse the user's reply as a duration (e.g. "2 hours", "90 min", "overnight" ≈ 8h) and compute `fireAt = now + duration`; if the reply is not interpretable as a duration, ask them to restate it as one. This first interval is the user's call because only they know how long this particular trace should be given to finish on its own before a safety resume kicks in. Use the runtime's scheduling primitive (`mcp__scheduled-tasks__create_scheduled_task`, or the harness equivalent) — its exact parameter schema is harness-supplied (load it via ToolSearch if the schema is not already in context); pass the computed future fire-time in whatever field that tool names for it, formatted as the tool's schema requires (do not assume a field name or format — read the schema). The wakeup's prompt is the canonical resume prompt below.
2. **Re-arm on each firing — fixed 5h thereafter (the self-perpetuating part).** When the wakeup fires, BEFORE doing recovery work it checks the ledger: if an `in-flight::` marker is still present (work remains), it **first schedules an identical wakeup `now + 5 hours`** (every interval AFTER the user-set first one is a fixed 5h — no re-asking), THEN resumes (runs Step 1 recovery on the marker). This chains the safety net across *repeated* deaths. If no marker is present (the trace converged or was cleanly handed off / stopped), it does **not** re-arm — the chain ends. **If the scheduling tool is unavailable at firing time** (the resumed session's toolset is harness-supplied and not guaranteed to carry it), skip the re-arm and tell the user that autonomous resume has ended and manual re-invocation is needed — mirroring the arm-time "skip arming if no mechanism available" fallback; do not block the resume itself.
...
> Resume `/skill-tracer <skill-name>`. First read `~/.claude/skill-tracer-audit-ledger/<skill-name>.md` for the `in-flight::` marker and run Step 1 recovery (the applicable rule). If a `dispatch`/`addressing` marker is present (work remains), FIRST re-arm an identical one-shot wakeup 5 hours from now (this prompt only ever runs on a *fired* wakeup, so the interval here is always the fixed 5h re-arm — the user-asked first interval applied only to the very first arm at Step 4, never to a re-arm), THEN resume the trace to convergence. If no such marker is present, the trace is done — do not re-arm.
