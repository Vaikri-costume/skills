...
1. **Templates (seed data, not live routing)** — `references/subfolder-templates.json` ships a *generic skeleton* (the five Q1 groupings + universal compound children). Each user's own taxonomy lives in a per-user override at `[root]/.organizer/templates.json`, which the backend **deep-merges** over the skeleton. Always load the merged result via `organizer.py templates` (not by cat-ing the shipped file) — it describes the top-level groupings (Q1) and what children compound subfolders like References/ and Financials/ contain (Q4), plus the canonical "in <folder>" description for every subfolder name. The file also carries a `parent_type_definitions`/`Q2_per_grouping` block sketching Q2/Q3 shape, but that block is reference/seed material only — no script reads it. **Actual Q2/Q3 routing at runtime is driven exclusively by the on-disk `.tidy-rules.json` cascade described below** (the live, evolving, user-specific rules), not by the shipped templates file. New users start from the bare skeleton and grow their taxonomy via the learning loop; the override file is written/extended as rules are learned.
...
2. propose          — backend prints raw pending-file records to stdout; Claude classifies + writes proposals_classified.json; also check `organizer.py inbox-list` → `{count, arbiter_trigger}` and run the inbox arbiter sweep if count reaches `arbiter_trigger` (configurable via config.json's `inbox_arbiter_trigger`, default 100; soft guideline — the sweep is correct at any count; the default simply amortises dispatch into ~4 arbiters of 25) (see "Inbox arbiter sweep" in the propose section — triggered here because a new batch is about to be classified and it's a good moment to reclaim _Inbox before adding more; compare with the process-return step 8b trigger which fires after returning viewer results). **Precondition (usually already satisfied, unlike step 8b which states it explicitly):** this trigger reads `inbox-list` counts that reflect the post-execute registry state. In the normal loop this is naturally the case — step 2 is only reached via "repeat from 1" (step 7), which itself only runs after execute (step 5) and cleanup (step 6) in the same pass — so propose-time inbox counts are already post-execute here. The one exception is the very first cycle's initial propose call, before any execute has ever run; at that point `_Inbox/` is necessarily still empty (nothing has been organised yet), so the count trivially cannot have reached `arbiter_trigger`, and the precondition is moot rather than violated.
...
The registry remembers everything across batches — original names, paths, hashes, content previews — so duplicates and variants are caught even if they appeared in different batches. (Every mutation also mirrors to `registry.csv` — see the intro.) On a brand-new drive, `scan` (via `get_db()`) auto-initialises `registry.db` on first run — there is no separate setup step. (The counters `scan` prints are informational read-outs of registry state at that moment, not a durable audit log — if `execute` is interrupted mid-batch, the registry itself stays authoritative and the next `scan`'s counters simply reflect whatever state the registry actually ended up in.)
...
- **These two counters are not exhaustive of every registry status.** A row can also land on `missing` (execute's crash-recovery journal path, when a file vanished mid-move) — that status is neither `pending` nor counted in `Skipped`, so this loop's stop condition can report "all batches are done" while a `missing` row exists unnoticed. See `references/subcommands.md` "status" for the full status set and what `missing` means; it is not a defect in this loop, just a status this counter pair doesn't surface — check `organizer.py status` if you suspect data loss. **Recovery path:** if a file marked `missing` later reappears on disk at its recorded path, `reconcile` detects it as issue type `missing_row_reappeared` (see `references/subcommands.md` "reconcile") and surfaces it for `--accept ID` — it is not permanently unreachable.
- `Pending classify = 0` **AND `Skipped` = 0 AND the run did not stop at a cap** ("→ All priorities drained within cap") → all batches are done; move to the final pass (`duplicates` → `variants` → `merge`). **Both counters are GLOBAL registry-wide counts** (`scan.py` computes them as `SELECT COUNT(*) FROM files WHERE status='pending'` with no scan- or batch-scoping in the `WHERE` clause) — not scoped to the current scan/batch. A "0 pending" reading reflects the *entire* registry's state at the moment scan finished, not just the rows this particular scan touched.
- `Pending classify = 0` **but `Skipped` > 0 or the run hit a cap** → work remains (downloads that failed, or files past the cap). Run `scan` again to pick them up. **Permanent-skip guard:** if two consecutive scans report the *same* `Skipped` count and add no new pending files, those files are persistently unavailable (online-only with no connectivity, zero-byte, or unreadable) — they will never drain. `scan` tracks this itself: it persists each run's `Skipped` count to a small sidecar file (`<root>/.organizer/.scan_skip_state.json`) and compares against it on the next run. When the guard fires, `scan` prints a line to stdout: `PERMANENT_SKIP_GUARD_TRIGGERED: <N> files persistently skipped`. You don't need to track or record anything yourself — just check `scan`'s output for that line. When it appears: stop re-scanning, tell the user which files keep skipping (the `skip <path>: …` stderr lines name them), and treat the drive as done *excluding* them — then **proceed to the final pass** (`duplicates` → `variants` → `merge`) exactly as in the fully-drained case above.
...
Caps each batch at 250 files OR 20GB cumulative size (whichever hits first) — configurable per-drive via the rules-viewer Settings panel (`scan_file_limit` / `scan_gb_limit` in `config.json`; `--limit`/`--limit-gb` still override per-run). `scan` walks files in this priority order, triggering cloud downloads inline when it reaches a cloud-only file inside the budget:
...
**`auto_approve` is overridable per-entity** (in `entities.json`, editable via the rules-viewer entity card): tri-state — absent/null inherits the global setting above; `true` always auto-approves that entity's W1 matches even if the global is off; `false` never auto-approves them even if the global is on.

**`min_token_len` is overridable per-entity** (in `entities.json`, editable via the rules-viewer entity card): overrides the global 4-character auto-classify token floor (3 for aliases) for that entity's own folder-name tokens. Set to `3` for short-name entities (IBM, BBC, ADM) that would otherwise never meet the global floor and so never auto-route. Absent/null inherits the global default.

**W1's "already in ruled folder" fast-path trusts path membership, not classification provenance.** `_auto_classify_entry` in `classify_propose.py` treats a file as "already correctly placed" purely because its *current path* sits under an active grouping whose rule resolves to that same folder — it does not check whether the file actually went through classify/propose review. A file placed there by hand, or restored via `reconcile --accept`, gets the identical W1 fast-path treatment (and, with `auto_approve` on, the identical auto-approval skipping the viewer) as one properly classified there. This is accepted behavior, not a bug: if the user deliberately filed something into a ruled folder, W1 recognizing that placement and not re-litigating it in the viewer is the intended save-effort case. It only becomes a mismatch if a file lands in a ruled folder by accident (e.g. an unrelated file happens to collide with a rule's destination) — the same review-skip applies regardless of intent, since the check has no way to distinguish "the user meant this" from "this coincidentally matched."
...
**Build `proposals_classified.json` (mechanized via `merge-proposals`).** `propose` prints the per-file records to **stdout** in `id` order; pass `--save-records <path>` to also persist that exact array to disk (purely additive — stdout is unchanged) so it can be fed forward. (`propose` also writes two *side* files regardless: the `project_metadata.json` sidecar always, and `auto-routed.csv` when anything auto-routed — never the proposals file itself.) Each saved record carries the base fields (`id`, `current_path`, `filename`, `extension`, `file_size`, `file_date`, `is_image`, `is_raw`, `content_peek`) **plus** its lane fields — `auto_routed` on every record, and per lane `para_subfolder`/`proposed_subfolder`/`auto_reason`/`auto_approved?` (auto-routed) or `needs_classification`/`classify_batch` (+`route_by_name_only`/`open_blocked_reason` when blocked). The two lanes are **disjoint by `id`** (a file is `auto_routed` OR `needs_classification`, never both).

After `--save-records`, dispatch the classify fan-out exactly as before (unchanged) and concatenate every batch's verdict array into one file (a flat JSON array, or a dict keyed by id — `merge-proposals` accepts either). Then run:

```
organizer.py merge-proposals --records <saved-records.json> --verdicts <verdicts.json> \
  [--reclassified <carry-forward.json>] [--arbiter <arbiter-entries.json>]
```

This performs the union deterministically and writes `~/.claude/drive-organizer/proposals_classified.json`: auto-routed records are kept as-is; `needs_classification` records are enriched in place with the matching verdict's `para_subfolder`/`new_filename`/`reason`/`signal`/`confidence`/optional `file_date`/`vision_desc` (a record with no matching verdict is left unenriched with a stderr warning, never dropped). `--reclassified` carries forward already-shaped entries from the previous viewer round (process-return step 3) unchanged. **Exception:** arbiter-sourced entries (`reroute_high`/`reroute_low`, built during the inbox arbiter sweep per `references/arbiter-prompt.md`) are a third, distinct entry shape that bypasses the classify fan-out entirely and carries neither `auto_routed` nor `needs_classification` — pass them via the optional `--arbiter <path>` flag rather than hand-splicing them in; `merge-proposals` merges them in unchanged, disjoint-by-id from every other lane. All lanes are validated disjoint-by-id; a genuine conflict (e.g. a record somehow both `auto_routed` and `needs_classification` — a bug in `propose`'s own output) errors out rather than silently picking a side. Prints a one-line summary (`Wrote proposals_classified.json: N auto-routed, M classified, K reclassified -> <path>`). You don't need to sort — `generate-viewer` bubble-sorts when it serves (`_bubble_sort_proposals`).
...
3. Now separate the remaining entries by action field, using the now-updated rules. `proposals_approved.json` contains **only** the four action values below — flagged and unreviewed rows are never written to it (flagged go to the registry + `proposals_flagged.json`; unreviewed go nowhere), so these four cases are exhaustive. **Fallback for a missing `action` field:** execute uses `entry.get("action", "approved")` — an entry with no `action` key is silently treated as `approved` and the file is moved to its `para_subfolder`. This does not occur in normal operation (the viewer always writes `action`) but is the code's behaviour for hand-edited JSON entries where the field is absent.
   ...
   - action='rejected'  → reclassify against the freshly-updated `.tidy-rules.json` (which now reflects everything learned in step 2). A rejection means the proposed destination was wrong — apply the updated rules to determine the correct folder. Do not send to `_Inbox/` unless no rule applies; that defeats the purpose of the rejection. **Scope note:** flipping `action` to `'approved'` here happens AFTER step 2's learn pass has already run for this round, so a reclassified-then-approved entry is not compared against the baseline for rule-learning in this same round — that comparison is deferred, not lost. The reclassified destination gets written into `proposals_classified.json` at step 9, which becomes the pre-edit baseline for the *next* round's viewer pass; if the user further edits that destination next round, step 2 will pick it up then. Nothing here bypasses learning permanently — it just spreads the comparison across rounds, same as any other entry that enters the pipeline after a round's baseline was captured. **When you rewrite a rejected entry, update `para_subfolder` (and `new_filename` if needed), carry `file_date` through unchanged, and flip `action` from `'rejected'` to `'approved'`** — the entry now has a correct destination, so it must read as approved or step 4's "remove remaining rejected entries" would drop it before execute. (If that `file_date` is null and you can now read the document's date, fill it. Carrying null does NOT simply skip the period update: execute falls back to the registry's already-stored `file_date` for that file id (`effective_date = file_date or the row's existing file_date`), so widening can still occur from the scan-time date; the period update is skipped only if BOTH the verdict's `file_date` and the registry's stored `file_date` are null/absent.) **If, after this one reclassify attempt — meaning one full pass through the cascading-Q rule cascade (Q1→Q2→Q3→Q4) against the current `.tidy-rules.json` state, not a per-level retry — no rule genuinely applies — the only case where `action='rejected'` may be left in place — do NOT leave it as `'rejected'`: flip `action` to `'inbox'` and set `para_subfolder='_Inbox'`, exactly like a viewer-side inbox action**, so the entry has a defined terminal fate (consistent with `_Inbox` being the skill's documented last-resort, never a silent drop) instead of being removed unrouted at step 4. This resolves the entry in this single pass — it is not re-attempted or carried forward to loop through step 3 again; a file that keeps landing here after future batches is user-paced (the user reviews and can reject it again next round), not an unbounded automated retry. execute uses `file_date` to widen the destination project's `date_range`; dropping it silently skips that update.
...
8. Re-run propose --limit 250 to pull new pending files.
   ...
   **Sync gate — mandatory before 9:** Wait for ALL classification agents dispatched in this step to return their verdicts and merge their output into a single classified list before proceeding to step 9. Do not proceed to step 9 before ALL classification agents have returned — **why:** step 9's merge requires every source to already carry the classified-entry shape, so proceeding early would merge an incomplete or malformed source.

8b. Run scan (this is the stop-condition oracle — `scan` is the command that prints `Pending classify`, `Skipped`, and the cap/`→ drained` stop-state line; `propose` has no cap and reports none of those counters). Also run `organizer.py inbox-list` → `{count, arbiter_trigger}` after scan, and if count reaches `arbiter_trigger` (config.json's `inbox_arbiter_trigger`, default 100) trigger the inbox arbiter sweep (see "Inbox arbiter sweep" in the propose section) before continuing. **Precondition:** run step 8b only after the execute sub-step (step 5) has completed, so `inbox-list` counts reflect the post-execute registry state (`inbox-list` counts only rows with `status='organized'` and a `_Inbox` path; files classified to `_Inbox/` this round but not yet executed are NOT counted). Now apply the branch table below. **The rows are ordered, mutually exclusive, and exhaustive: evaluate top to bottom and act on the FIRST row whose condition holds, then stop — later rows do not apply.** First run the inbox-count check (rows 1–2), then evaluate the stop-condition rows (rows 3–7) in order:

   | # | Condition | Action |
   |---|-----------|--------|
   | 1 | **Inbox-count check (run FIRST):** inbox-list count >= arbiter_trigger | Trigger arbiter sweep now (before the stop-condition rows); run all arbiters in parallel; apply their verdicts; then re-run `organizer.py inbox-list` to confirm count dropped. Then fall through to the stop-condition rows below. |
   | 2 | inbox-list count < arbiter_trigger | Skip the arbiter sweep; fall through to the stop-condition rows below. |
   | 3 | **Stop condition NOT met**, evaluated as: (Pending classify > 0) OR run stopped at a cap OR (Skipped > 0 AND the Permanent-skip guard has NOT triggered — i.e. this is not yet the second consecutive scan with the same Skipped count) | Continue the loop: proceed to step 9 to merge, then step 10 to surface the next viewer batch. *(All remaining rows assume the stop condition IS met — either "Pending classify = 0 AND Skipped = 0 AND run did NOT stop at a cap" (fully drained), OR "Pending classify = 0 AND run did NOT stop at a cap AND Skipped > 0 WITH the Permanent-skip guard triggered" (persistently-skipped case, row 5). The Permanent-skip guard is evaluated BEFORE this plain Skipped>0 check, so a drive with persistently-skipped files never lands in row 3 — it falls through to row 5 instead.)* |
   | 4 | Stop condition MET AND **NEW entries pending viewer surfacing** exist this round — reclassified (from step 3) / flagged (from step 7) / reroute_low (from the row-1 arbiter sweep, if it ran) | Run steps 9–10 to merge and surface those freshly reclassified/flagged/reroute_low entries in the viewer — the drive is not sorted while they await review; do NOT skip to "drive is sorted". Note: directly-executed reroute_high entries from the arbiter do NOT require a viewer step — they count as done, not as "new entries pending viewer surfacing". |
...
After each successful file move, execute calls `_expand_date_range` on the destination project (walking ancestors via `_find_project_for_destination` to find a folder whose `.tidy-rules.json` carries a `filename_tag` **or** a `date_range` — a date_range-only folder such as an area/event/tax-year is a valid dated destination too, matching `_enumerate_project_metadata`). It widens `date_range` to encompass the file's date, with `buffer_days` set from `period_buffer_days` — a per-drive configurable override in `config.json`, editable in the rules-viewer Settings panel, defaulting to 30 (one calendar month) on each side when unset. First file in a fresh project initialises the period; subsequent files only widen it if their date falls outside the existing range.

**Skip conditions (all five must clear, else widening is silently skipped):** (1) `action='delete'` — the file went to the `Archive/_To Delete/` staging area, which is NOT a project folder and carries no `date_range`, so there is nothing to widen (this skip is deliberate, not an oversight — a future editor must not "fix" it by widening a delete entry's range); (2) no known date (neither the verdict nor the registry has a `file_date`); (3) date is before 1990-01-01 — treated as unreliable / pre-digital noise (a legit pre-1990 file will not widen its project's date range); (4) date is more than 365 days in the future — treated as a clock/metadata error; (5) no ancestor `.tidy-rules.json` carries a `filename_tag` or `date_range` for this destination — `_find_project_for_destination` walks up from the destination subfolder looking for such an ancestor, and if none exists it returns `None`, silently skipping the widening call entirely (there is no dated project folder to widen). The 1990-01-01 floor and 365-day ceiling are configurable per-drive via the rules-viewer Settings panel (`date_floor` / `date_ceiling_days` in `config.json`; the ceiling stays relative to "now", so it always tracks the current date). Current code: `_expand_date_range(... buffer_days=_effective_period_buffer_days(drive))` — `_expand_date_range` lives in `scripts/drive_organizer/date_range.py` (default param `buffer_days=30`, used only when no per-drive override is passed) and `_effective_period_buffer_days` lives in `scripts/drive_organizer/paths_config.py`. Confirm against the function signatures when editing.
...
If the script exits with `Error: root path not found: <path>` — see the troubleshooting note in `references/subcommands.md`.
...
- **`rules-viewer`** (`--port 5003`) — browser editor: clustered cards (250/session, 25/page — page size configurable via `viewer_page_size`, see Settings panel below; the 250/session entity cap is fixed), per-entity type/aliases/relation/behaviour/notes + signal, usage stats, dead-rule flag, why-routed, conflict warnings, test-a-file, coverage gaps, full CRUD + rename/merge/bulk, **rethink** (flag for re-inference, ≠ delete), area add/rename/remove, level-promotion (**dry-run preview only** — the plan endpoint returns the steps to move an entity up/down a level; there is no Apply wiring for it, so the user carries out the folder move + rules-file edit + `reconcile` manually, or uses the reconcile move machinery directly — see `entities_rules.py`'s `_promotion_plan` docstring), **Apply (keep open)** / **Preview** (per-change undo) / **Save & close**. Adapts to light/dark. (Implementation note for maintainers: each page load does two independent `.tidy-rules.json` tree walks — one for the entity view, one for conflicts/coverage — kept separate on purpose so the viewer's display logic never couples to the W1 matcher's hot path. This is a manual, localhost-only viewer opened a handful of times, not a production hot path, so the two-walk cost is intentional; the rationale also lives at the `do_GET` comment in `rules_viewer.py`. Do not "optimise" by merging the walks.)
  - **⚙ Settings panel** — a collapsible panel that reads/writes the per-drive `<root>/.organizer/config.json` settings: `peek` / `vision` (model_capabilities), `auto_approve`, `skip_types`, `skip_over_mb`, `variant_tokens` (extra merge-candidate vocabulary — see "variants" final pass), `classify_batch_size` (fan-out batch size, default 25), `period_buffer_days` (date_range widening buffer, default 30), `content_peek_chars` (content-peek excerpt length, default 300), `download_poll_timeout` (cloud-download poll timeout), `inbox_arbiter_trigger` (see "Inbox arbiter sweep", default 100), `scan_file_limit` / `scan_gb_limit` (per-batch caps, default 250 / 20), `date_floor` / `date_ceiling_days` (see "Skip conditions" under execute), `viewer_page_size` (rows per page in this viewer and the proposal-review viewer; default 25), `atomic_signatures_extra` (extra atomic-unit folder names/suffixes for this drive — comma-separated, extends the shipped list, never replaces it). "Save settings" POSTs to a separate `/config` endpoint — independent of the rule-edit Save/Apply, so changing settings never touches the rule set. This panel is the single settings surface: any new toggle/dial added later is wired in here too. (Per-entity overrides — `filename_tag`, `min_token_len`, `date_range`, `auto_approve` — live on the entity card, not this panel; see the `auto_approve` per-entity note below.)
...
1. **`bootstrap --detect-atomic`** — lists atomic-unit folders (venvs, `node_modules`, `.git` repos, Zotero/photo libraries, `.app` bundles, Unity projects, CPAP `OSCAR_Data` exports, Time Machine `Backups.backupdb`). Signatures are data-driven from `references/atomic-signatures.json` (dir names, suffixes, marker-file/marker-pair probes) plus any per-drive extra names/suffixes set in the rules-viewer Settings panel or `<root>/.organizer/config.json`'s `atomic_signatures_extra` — see `references/glossary.md` "atomic-unit folder". Show them to the user. **`atomic-signatures.json`'s probe lists are open, not closed** — adding a new `dir_names`/`suffixes`/`marker_files`/`marker_pairs` entry is a routine edit and needs no other file touched. `paths_config.py`'s hardcoded `_ATOMIC_*_FALLBACK` constants (used only when the shipped JSON is missing/unparseable) are deliberately NOT kept in sync with this JSON — they're a frozen last-resort minimal set matching the tool's original hardcoded behavior, not a mirror that must track every addition here.
