...
#### Added
- **Viewer screenshot asset** — `assets/viewer.png`, a screenshot of the browser proposal-review
  viewer (grouped destinations, inline-editable folder/filename fields, per-row approve / reject /
  flag / inbox / delete), referenced from the README; `assets/README.md` describes it.
...
- No behaviour change — `organizer.py` is untouched; documentation/structure only. (Versioned as a
  minor per the feature-ladder plan; strict SemVer would treat a docs/structure-only change as a
  patch.)
